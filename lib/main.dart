...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    home: MyHomePage(title: 'BLE Flutter App'),
  );
}
...
class MyHomePageState extends State<MyHomePage> {
  ...
  ListView _buildListViewOfDevices() {
    ...
    // Filter devices starting with 'PUL'
    final filteredDevices = widget.devicesList.where(
          (device) => device.advName.startsWith('PUL'),
    );

    for (BluetoothDevice device in filteredDevices) {
      containers.add(
        SizedBox(
          height: 50,
          child: Row(
